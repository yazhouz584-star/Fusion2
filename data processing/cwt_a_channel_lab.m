...
% 创建通道文件夹
mkdir(fullfile(dataset_dir, "train", "R"));
mkdir(fullfile(dataset_dir, "train", "G"));
mkdir(fullfile(dataset_dir, "train", "B"));
mkdir(fullfile(dataset_dir, "valid", "R"));
mkdir(fullfile(dataset_dir, "valid", "G"));
mkdir(fullfile(dataset_dir, "valid", "B"));
mkdir(fullfile(dataset_dir, "test", "R"));
mkdir(fullfile(dataset_dir, "test", "G"));
mkdir(fullfile(dataset_dir, "test", "B"));
...
for iFile = 1:numel(MAT_FILES)
    ...
    for iCut = 1:Samples
        ...
        imgCWT1 = imresize(imgCWT1, [ImageL, ImageW]);
        imgCWT2 = imresize(imgCWT2, [ImageL, ImageW]);
        imgCWT3 = imresize(imgCWT3, [ImageL, ImageW]);
        ...
        % 保存R通道图片
        save_path_R = fullfile(dataset_dir, folder, "R", strcat(num2str(iCut), '-', fault_label, '-R.png'));
        imwrite(imgCWT1, save_path_R);

        % 保存G通道图片
        save_path_G = fullfile(dataset_dir, folder, "G", strcat(num2str(iCut), '-', fault_label, '-G.png'));
        imwrite(imgCWT2, save_path_G);

        % 保存B通道图片
        save_path_B = fullfile(dataset_dir, folder, "B", strcat(num2str(iCut), '-', fault_label, '-B.png'));
        imwrite(imgCWT3, save_path_B);
    end
end
